...
def sim_decision_making_network_full(N_Excit = 400, N_Inhib = 100, weight_scaling_factor = 5.33,
    ...
    #tau_NMDA_s = 0.0 * b2.ms
    tau_NMDA_x = 2.0 * b2.ms
    #tau_NMDA_x = 0.0 * b2.ms
    alpha_NMDA = 0.5 * b2.kHz

    [AMPAtoNMDAratio1, AMPAtoNMDAratio2] = AMPAtoNMDAratio
    # projections from the external population
    g_AMPA_extern2inhib = AMPAtoNMDAratio1 * 1.62 * b2.nS
    g_AMPA_extern2excit = AMPAtoNMDAratio1 * 2.1 * b2.nS
    ...
    g_AMPA_excit2excit = AMPAtoNMDAratio1 * weight_scaling_factor * 0.012 * b2.nS
    g_AMPA_excit2inhib = AMPAtoNMDAratio1 * weight_scaling_factor * 0.015 * b2.nS
    g_NMDA_excit2excit = AMPAtoNMDAratio2 * weight_scaling_factor * 0.040 * b2.nS
    g_NMDA_excit2inhib = AMPAtoNMDAratio2 * weight_scaling_factor * 0.045 * b2.nS  # stronger projection to inhib.
...
threshold_gg = 15
different_reaction_time = []
for AMPAtoNMDAratio in [[1, 1], [70, 1/70]]:
	reaction_time = []
	for neverused in range(5):
		print(AMPAtoNMDAratio, neverused)
		...
		if AMPAtoNMDAratio == [1, 1]: hue = 'red'
		else: hue = 'blue'
		plt.plot(tA, rA, color = hue)
		plt.plot(tB, rB, color = hue)
		plt.xlabel('Time(ms)')
		plt.ylabel('Rate(Hz)')
	print('Avg reaction time =',np.average(reaction_time))
	different_reaction_time.append(np.average(reaction_time))
print(different_reaction_time)
plt.show()